...
struct FillingData {
    static let data: [ContentType] = [
        //TODO: fix paths for stories 5 and 6
        .gallery(Gallery(coverImage: .init("games-0"), title: "Board Games", images: .init(base: "games", count: 5))),
        .story(Story(coverImage: .init("story-6"), title: .from("s6-title"), text: .from("s6-text"), paths: [.story4path1, .story4path2])),
        .gallery(Gallery(coverImage: .init("books-0"), title: "Books", images: .init(base: "books", count: 5))),
        .story(Story(coverImage: .init("story-5"), title: .from("s5-title"), text: .from("s5-text"), paths: [.story4path1, .story4path2])),
...
struct Story: ContentRepresentable {
...
struct Gallery: ContentRepresentable {
    let coverImage: UIImage
    let title: String
    let images: [UIImage]
